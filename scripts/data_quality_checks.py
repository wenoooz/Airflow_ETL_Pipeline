...
def check_row_count(df: pd.DataFrame, expected_size: int) -> dict:
    ...
    actual = int(len(df))
    passed = bool(actual == expected_size)
    return {
...
def check_metric_ranges(df: pd.DataFrame) -> dict:
    ...
    invalid_ber = int((~ber_ok).sum())
    invalid_bler = int((~bler_ok).sum())
    invalid_thr = int((~thr_ok).sum())
    passed = bool(invalid_ber == 0 and invalid_bler == 0 and invalid_thr == 0)
    return {
        "check": "metric_ranges_valid",
        "passed": passed,
        "invalid_ber_count": invalid_ber,
        "invalid_bler_count": invalid_bler,
        "invalid_throughput_count": invalid_thr,
...
def check_snr_matches_plan(df: pd.DataFrame, expected_snr: list) -> dict:
    ...
    passed = bool(actual_snr == expected_set)
    return {
        "check": "snr_values_match_plan",
        "passed": passed,
        "expected_snr": sorted(int(x) for x in expected_set),
        "actual_snr": sorted(int(x) for x in actual_snr),
